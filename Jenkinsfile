    ...
    stages {
        stage('Scan Base Image') {
            steps {
                ...
                sh label: '', script: 'trivy image --format template --template @junit.tpl -o base-image.xml centos:8'
        ...
        stage('Scan App Image') {
            steps {
                sh label: '', script: 'trivy image --exit-code 1 --severity CRITICAL,HIGH --format template --template @junit.tpl -o app-image.xml thinknyx/devopsinaction:1.0'
